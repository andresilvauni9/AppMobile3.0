...
Write-Host "`n📋 Verificando pré-requisitos..." -ForegroundColor $infoColor
...
# Build da web app
Write-Host "`n🔨 Step 1/3: Build web app..." -ForegroundColor $infoColor
npm run build
...
# Sync com Android
Write-Host "`n🔄 Step 2/3: Sincronizar com Android..." -ForegroundColor $infoColor
npx cap sync android
if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ Erro ao sincronizar" -ForegroundColor $errorColor
    exit 1
}
Write-Host "✅ Sincronização completa" -ForegroundColor $successColor

# Build do Android
Write-Host "`n📱 Step 3/3: Build Android APK..." -ForegroundColor $infoColor
cd android

if ($release) {
    Write-Host "   Gerando APK de RELEASE (produção)..." -ForegroundColor $warningColor
...
if (Test-Path $apkPath) {
    ...
    Write-Host "`n📦 Localização: $apkPath" -ForegroundColor $successColor
    ...
    if ($release) {
        ...
        Write-Host "`n📱 Para instalar em dispositivo conectado:" -ForegroundColor $infoColor
...
Write-Host "`n✨ Build completo!" -ForegroundColor $successColor
